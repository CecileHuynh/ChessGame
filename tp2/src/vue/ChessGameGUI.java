...
public class ChessGameGUI extends JFrame implements MouseListener, MouseMotionListener, Observer{
	...
	private void clearGrid() {
		for (Component c : chessBoardGuiContainer.getComponents()) {
			if (c instanceof Container) {
				((Container) c).removeAll();
			}
			// chessBoardGuiContainer.remo
		}
	}
	...
	public void update(Observable o, Object arg) {
		...
		JLabel piece;
		JPanel panel;
		List<PieceIHM> newList=new LinkedList<>();
		newList=listPieceIHM;
		Iterator<PieceIHM> it=newList.iterator();
		while(it.hasNext()){
			PieceIHM cp=it.next();
			piece = new JLabel(new ImageIcon(ChessImageProvider.getImageFile(cp.getName(), cp.getCouleur())));
			panel = (JPanel) chessBoardGuiContainer.getComponent((cp.getX()) + (cp.getY() * 8));
			panel.add(piece);
		}
		/*
		for (int i = 0; i < listPieceIHM.size(); i++) {

				piece = new JLabel(new ImageIcon(ChessImageProvider.getImageFile(listPieceIHM.get(i).getName(), listPieceIHM.get(i).getCouleur())));
				panel = (JPanel) chessBoardGuiContainer.getComponent((listPieceIHM.get(i).getX()) + (listPieceIHM.get(i).getY() * 8));
				panel.add(piece);
		}*/
	}
